		...
		<fieldset>
			<form action="{{ route('updateVideo', ['id' => $video->id]) }}" method="POST" enctype="multipart/form-data">
				...
				<div class="form-group">
					...
					<input type="text" name="title" class="form-control  @error('title') is-invalid @enderror" id="video_title" value="{{ old('title', $video->title) }}">
				...
				<div class="form-group">
					...
					<textarea class="form-control @error('description') is-invalid @enderror" name="description" id="video_description" rows="4">{{ old('description', $video->description) }}</textarea>
				...
				<div class="form-group">
					...
					<select class="form-control select_video_source @error('video_source_name') is-invalid @enderror" name="video_source_name" id="video_source_name">
						<option value="youtube" {{ old('video_source_name') === 'youtube' ? 'selected' : ($video->video_source_name === 'youtube' ? 'selected' : '') }}>Youtube</option>
						<option value="vimeo" {{ old('video_source_name') === 'vimeo' ? 'selected' : ($video->video_source_name === 'vimeo' ? 'selected' : '') }}>Vimeo</option>
						<option value="direct_upload" {{ old('video_source_name') === 'direct_upload' ? 'selected' : ($video->video_source_name === 'direct_upload' ? 'selected' : '') }}>Direct Upload</option>
					</select>
				...
				<div class="form-group source_field">
					...
					<input type="text" name="video_source" class="form-control @error('video_source') is-invalid @enderror" id="youtube" value="{{ old('video_source', $video->video_source) }}">
				...
				<div class="form-group d-none source_field mb-3">
					...
					<input type="text" name="video_source" class="form-control @error('video_source') is-invalid @enderror" id="vimeo" disabled="disabled" value="{{ old('video_source', $video->video_source) }}">
				...
				<div class="form-group">
					<button type="submit" class="btn btn-primary ripple m-1">update</button>
...
$(document).ready(function() {
	...
	var selected_category = @json($video->category_id);
	...
	$('.select_category').val([selected_category]).change();
